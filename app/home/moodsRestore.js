...
export default function Restore() {
  ...
  useEffect(() => {
    async function load() {
      const all = await getDeletedMoods();
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Restore Moods</Text>
      ...
      {startDate && endDate && (
        ...
        >
          <Text style={[styles.deleteText, { backgroundColor: "#80231f" }]}>
            Clear filter
          </Text>
        </TouchableOpacity>
      )}
          ...
          displayedEntries.map((entry) => (
            <View key={entry.date} style={styles.entryCard}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  await saveMood(entry.date, {
                    mood: entry.mood,
                    note: entry.note,
                  });

                  await removeDeletedMood(entry.date);
